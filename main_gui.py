...
class MainGUI(QWidget):
    ...
    def get_version_text(self):
        return (
            f"KiCad: {self.package_versions.get('kicad', 'Unknown')}\n"
    ...
    def runUpdate(self, script):
        try:
            ...
            QMessageBox.critical(self, "Error", f"An error occurred while running the script: {e}")
    ...
    def packages_downloaded(self):
        """Check if the packages have been downloaded by verifying the nghdl/packages folder exists and is non-empty."""
        package_dir = os.path.join("nghdl", "packages")
        return os.path.exists(package_dir) and os.path.isdir(package_dir) and bool(os.listdir(package_dir))
    
    def openGHDL(self):
        if not self.packages_downloaded():
            QMessageBox.critical(self, "Error", "Please download the packages first.")
            return
        # Assuming GHDLUpdaterGUI is defined elsewhere
    ...
    def openVerilator(self):
        if not self.packages_downloaded():
            QMessageBox.critical(self, "Error", "Please download the packages first.")
            return
        # Assuming VerilatorUpdaterGUI is defined elsewhere
    ...
    def openNGSpice(self):
        if not self.packages_downloaded():
            QMessageBox.critical(self, "Error", "Please download the packages first.")
            return
        # Assuming NGSpiceUpdaterGUI is defined elsewhere
        self.ngspice_window = NGSpiceUpdaterGUI(self)
        self.ngspice_window.show()
    
    def openKiCad(self):
        # KiCad updater may not require the downloaded packages check
